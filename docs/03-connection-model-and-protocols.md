...
- 断线后不自动无限重连，首版采用手动重连。
- 关闭文件 Session 会释放 SFTP 会话；传输任务状态由传输队列继续展示结果。
...
Windows 下通过后端检测 WSL，作为跨平台能力的一部分：
...
- host key 确认结果需要持久化。
- host key 错误由 Rust 返回结构化 payload，前端识别后弹出确认 Dialog。
...
FTP/FTPS 用于文件传输，不提供终端 shell。当前仅在连接模型和 UI 中保留协议类型，后端连接与文件操作尚未实现。
...
当前 Rust 错误通过 JSON 字符串传回前端，包含 `code`、`message`、`details` 和可选 `hostKey`。前端使用 `parsePuckError` 解析，并通过 `errors` i18n namespace 映射用户文案。
